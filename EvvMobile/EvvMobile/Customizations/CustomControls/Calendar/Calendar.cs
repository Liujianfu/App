...
namespace EvvMobile.Customizations.CustomControls.Calendar
{
	public partial class Calendar : ContentView
	{
        ...
        public static double GridSpace1 = 0;
		public event EventHandler<DateTimeEventArgs> OnStartRenderCalendar, OnEndRenderCalendar;
        ...
        public Calendar()
		{
			...
			MonthNavigationLayout = new StackLayout
			{
				...
				HeightRequest = Device.RuntimePlatform!= Device.iOS&& Device.RuntimePlatform != Device.Android ? 50 : 30,
        ...
        public static readonly BindableProperty GridSpaceProperty =
            BindableProperty.Create(nameof(GridSpace), typeof(double), typeof(Calendar), 0.0,
                propertyChanged: (bindable, oldValue, newValue) => (bindable as Calendar).ChangeGridSpace((double)newValue, (double)oldValue));

        
        protected void ChangeGridSpace(double newValue, double oldValue)
        {
            if (newValue == oldValue) return;
            MainCalendars.ForEach(c => { c.RowSpacing = newValue; c.ColumnSpacing = newValue; });
        }
        public double GridSpace
        {
            get { return (double)GetValue(GridSpaceProperty); }
		...
		{
            var start = date;
		    if (CalendarViewType == DateTypeEnum.Week)
		    {
		...
		protected void CreateButtons()
		{
			...
			for (var i = 0; i < ShowNumOfMonths; i++)
			{
				var mainCalendar = new Grid { VerticalOptions = LayoutOptions.FillAndExpand, HorizontalOptions = LayoutOptions.FillAndExpand, RowSpacing = GridSpace, ColumnSpacing = GridSpace, Padding = GridSpace, BackgroundColor = DatesBackgroundColor };
			    ...
			    {
			        numberOfWeek = 1;
			        mainCalendar.RowDefinitions = new RowDefinitionCollection { rowDef };
                }
				for (int r = 0; r < numberOfWeek; r++)
				{
					for (int c = 0; c < 7; c++)
					{
        ...
        protected void ChangeCalendar(CalandarChanges changes)
        {
			...
			{
				OnStartRenderCalendar?.Invoke(this, new DateTimeEventArgs { DateTime = CalendarStartDate(StartDate), IsWeekView = CalendarViewType == DateTypeEnum.Week });
				Content = null;
				...
				Content = MainView;
				OnEndRenderCalendar?.Invoke(this, new DateTimeEventArgs { DateTime = CalendarStartDate(StartDate), IsWeekView = CalendarViewType == DateTypeEnum.Week });
			});
        }
		...
		{
		    var calendarButton = s as CalendarButton;
		    if (calendarButton != null)
		    {
		        var selectedDate = calendarButton.Date;
