			...
			Device.BeginInvokeOnMainThread(() =>
			{
			    if (!MultiSelectDates)
			    {
                    ...
                    if(button.Date.HasValue)
			            SelectedDates.Add(button.Date.Value.Date);
                }
		...
		protected bool ChangeSelectedDate(DateTime? date, bool clicked = true)
		{
			if (!date.HasValue) return false;
			    ...
			    else
			    {
				    ...
				    var spD = SpecialDates?.FirstOrDefault(s => s.Date.Date == date.Value.Date);
			    ...
			    if (clicked &&!deselect)
			    {
			        ...
			        var selectedIndex = (date.Value - start).Days;

                    DateClicked?.Invoke(this, new DateTimeEventArgs { DateTime = date.Value ,SelectedIndex = selectedIndex });

				    DateCommand?.Execute(new DateTimeEventArgs { DateTime = date.Value, SelectedIndex = selectedIndex });
			    }
			    return deselect;	        
