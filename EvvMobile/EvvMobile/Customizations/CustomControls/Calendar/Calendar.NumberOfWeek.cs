		...
		protected void ShowHideElements()
		{
			...
			for (var i = 0; i < ShowNumOfMonths; i++)
			{
				...
				if (WeekdaysShow)
				{
					...
					var dl = new Grid { VerticalOptions = LayoutOptions.Center, RowSpacing = 0, ColumnSpacing = 0, Padding =0 };
				 //   if (Device.RuntimePlatform != Device.iOS && Device.RuntimePlatform != Device.Android)
				    {
				        var rowDef = new RowDefinition { Height = NumberOfWeekFontSize * 1.5+6 };
                        dl.RowDefinitions = new RowDefinitionCollection { rowDef };				        
				    }
				    ...
				    if (Device.RuntimePlatform != Device.iOS && Device.RuntimePlatform != Device.Android)
				    {
				        marginFront = NumberOfWeekFontSize * 2.5;
				    }
