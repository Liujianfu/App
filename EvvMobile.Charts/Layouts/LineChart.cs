namespace EvvMobile.Charts.Layouts
{
    ...
    public class LineChart : PointChart
    {
        ...
        public override void DrawContent(SKCanvas canvas, int width, int height)
        {
            ...
            var xOrigin = this.MarginLeft + (itemSize.Width / 2);
            this.DrawAxises(canvas, xOrigin, height- footerHeight-this.Margin, headerHeight,footerHeight, width, height);
        }

        protected void DrawAxises(SKCanvas canvas,float xOrigin, float yOrigin,float headerHeight, float footerHeight,float width,float height)
        {
            DrawGridLine(canvas, xOrigin, yOrigin, xOrigin, headerHeight,AxisColor);
            DrawGridLine(canvas, xOrigin, yOrigin, width - this.Margin, yOrigin, AxisColor);
            DrawYValueLabel(canvas, xOrigin, height, headerHeight, footerHeight);
        }
                    ...
                    using (var shader = this.CreateGradient(points))
                    {
                        ...
                        for (int i = 0; i < last; i++)
                        {
                            if (this.LineMode == LineMode.Spline)
                            {
                                ...
                                this.CalculateCubicInfo(points, i, itemSize, ref path);
                            }
        ...
        private void CalculateCubicInfo(SKPoint[] points, int i, SKSize itemSize, ref SKPath path)
        {
            var point = points[i];
            ...
            path.CubicTo(currentControl, nextControl, nextPoint);
