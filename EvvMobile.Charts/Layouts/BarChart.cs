        ...
        protected void DrawBars(SKCanvas canvas, SKPoint[] points, SKSize itemSize, float origin, float headerHeight)
        {
            ...
            if (points.Length > 0)
            {
                for (int i = 0; i < this.Entries.Count(); i++)
                {
                    ...
                    if (entry.Value>0 &&entry.SubEntries != null && entry.SubEntries.Any())
                    {
                        ...
                        foreach (var subEntry in entry.SubEntries)
                        {
                            ...
                            {
                                var subHeight = (subEntry.Value / entry.Value) * height;
                                

                                var rect = SKRect.Create(x, y, itemSize.Width, subHeight);
